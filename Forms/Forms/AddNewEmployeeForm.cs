...
internal class AddNewEmployeeForm : Form
{
    ...
    private TextBox _mailAddressBox = null!;
    ...
    private async Task MakeMailAddressBoxAsync()
    {
        _mailAddressBox = new TextBox()
        {
        ...
        _mailAddressBox.KeyPress += new KeyPressEventHandler(CorrectKeyPress!);
        _mailAddressBox.Enter += (sender, args) =>
        {
            if (sender is TextBox mailAddress)
                RemoveErrorLabel(mailAddress.Location.X - 15, mailAddress.Location.Y);
        };

        Controls.Add(_mailAddressBox);
        await Task.CompletedTask;
    }
    ...
    private void CorrectKeyPress(object sender, EventArgs args)
    {
        ...
        {
            if (textBox.TabIndex != 0 && keyPress.KeyChar == ' ') keyPress.KeyChar = '\0';
    ...
    private bool TryMakeLabelRequiredField()
    {
        ...
        try
        {
            _ = new MailAddress(_mailAddressBox.Text);
        }
        catch (Exception)
        {
            flag = TryMakeErrorLabel(_mailAddressBox.Location.X - 15, _mailAddressBox.Location.Y);
        }
